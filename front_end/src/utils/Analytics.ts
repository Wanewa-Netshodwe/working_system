...
const calculateMetrics = (data: AttendanceData[]) => {
  ...
  data.forEach((record) => {
    if (isOnTime(new Date(record.clock_in))) {
      ontime++;
      ontime_dataset.push(
        new Date(record.clock_in).getHours() +   new Date(record.clock_in).getMinutes()
      );
    } else {
      late++;
      late_dataset.push(
    ...
    working_hours_dataset.push(record.workHours.hours);
...
const calculateMetricsDaily = (data: AttendanceData) => {
  console.log("calculateMetricsDaily - Processing single record:", JSON.stringify(data));
  ...
  if (isOnTime(new Date(data.clock_in))) {
    ontime++;
    ontime_dataset.push(new Date(data.clock_in).getHours() + new Date(data.clock_in).getMinutes());
  } else {
    late++;
    late_dataset.push(new Date(data.clock_in).getHours() + new Date(data.clock_in).getMinutes());
  }
  lunch_hours += 1;
  lunch_hours_dataset.push(1);
  working_hours = {
